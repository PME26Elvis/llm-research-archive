...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'desktop-import-session-'));
  roots.push(root);
...
describe('ImportSessionService', () => {
  it('returns a sanitized preview and never exposes source write paths', () => {
    ...
    expect(JSON.stringify(result.preview)).not.toContain(path.dirname(sourcePath));
    expect(result.preview.targetArticleRelativePath).toBe('llm/gpu-model-report/index.md');
  });
  ...
  it('commits only cached plans and retains the source by default', () => {
    const { workspace, sourcePath } = fixture();
    const service = new ImportSessionService();
    expect(service.commit({ planId: 'a'.repeat(64), removeSource: false }, workspace)).toEqual({
      ok: false,
      code: 'plan-not-found',
      message: '匯入預覽已過期；請重新建立預覽。',
    });
    ...
    expect(fs.existsSync(path.join(workspace.rootPath, 'llm/gpu-model-report/index.md'))).toBe(
      true,
    );
  });
