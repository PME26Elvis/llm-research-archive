...
function displayPath(plan: ImportPlan, candidate: string | undefined): string | undefined {
  if (!candidate) return undefined;
  const absolute = path.resolve(candidate);
  if (plan.source.kind === 'article-folder' && isInside(plan.source.rootPath, absolute)) {
    const relative = path.relative(plan.source.rootPath, absolute).split(path.sep).join('/');
    return relative || path.basename(plan.source.rootPath);
  }
  if (isInside(plan.workspaceRoot, absolute)) {
    const relative = path.relative(plan.workspaceRoot, absolute).split(path.sep).join('/');
...
function rejectedPreview(
  code: 'workspace-read-only' | 'plan-not-found' | 'invalid-source' | 'invalid-metadata',
  message: string,
): ImportPreviewResult {
...
): Exclude<Extract<ImportSessionCommitResult, { ok: false }>['code'], 'plan-not-found'> {
  return code;
}
...
export class ImportSessionService {
  ...
  createPreview(sourcePath: string, workspace: WorkspaceInfoDto): ImportPreviewResult {
    if (workspace.kind !== 'local') {
      return rejectedPreview('workspace-read-only', '內建封存為唯讀；請先開啟可寫入的本機工作區。');
    }
    ...
    if (!result.ok) {
      const metadataFailure = result.issues.every((issue) => issue.code === 'invalid-metadata');
  ...
  refreshPreview(
    ...
    if (!current || path.resolve(current.workspaceRoot) !== path.resolve(workspace.rootPath)) {
      return rejectedPreview('plan-not-found', '匯入預覽已過期；請重新選擇來源。');
    }
    const result = createImportPlan({
  ...
  commit(request: ImportCommitRequest, workspace: WorkspaceInfoDto): ImportSessionCommitResult {
    if (workspace.kind !== 'local') {
      return {
        ok: false,
    ...
    if (!plan || path.resolve(plan.workspaceRoot) !== path.resolve(workspace.rootPath)) {
      return {
        ok: false,
        code: 'plan-not-found',
