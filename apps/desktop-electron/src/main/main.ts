...
function bundledContentRoot(): string {
  if (!app.isPackaged)
    return (
      process.env.ARCHIVE_CONTENT_ROOT || path.resolve(__dirname, "../../docs")
    );
...
function initializeWorkspace(): void {
  ...
  if (process.env.ARCHIVE_CONTENT_ROOT) {
    const selected = validateWorkspaceRoot(
      path.resolve(process.env.ARCHIVE_CONTENT_ROOT),
    );
...
function validateSender(
  ...
  if (frame && frame !== sender.mainFrame)
    throw new Error("invalid-ipc-subframe");
  const actual = frame?.url || sender.getURL();
  const actualOrigin = actual ? new URL(actual).origin : "";
  if (trustedRendererOrigin) {
    if (actualOrigin !== trustedRendererOrigin)
      throw new Error(`invalid-ipc-sender:${actual}`);
    return;
  }
  if (actual !== trustedRendererUrl)
    throw new Error(`invalid-ipc-sender:${actual}`);
}
function installApplicationMenu(win: BrowserWindow): void {
  ...
  const template: MenuItemConstructorOptions[] = [
    ...
    },
    {
      label: "說明",
      submenu: [
        { label: "關於 Research Observatory", click: () => send("about.open") },
      ],
    },
  ];
  ...
  session.defaultSession.protocol.registerFileProtocol(
    ...
    (request, callback) => {
      safeAssetPath(request.url)
        .then((filePath) =>
          callback(filePath ? { path: filePath } : { error: -10 }),
        )
...
ipcMain.handle("archive:list", (event) => {
  ...
  return ArticleListResponseSchema.parse(
    core.listArticles().map(summarizeArticle),
  );
});
ipcMain.handle("article:get", (event, p) => {
  validateSender(event.sender, event.senderFrame);
  return ArticleDtoSchema.parse(
    core.getArticle(ArticleRequestSchema.parse(p).id),
  );
});
ipcMain.handle("search:query", (event, p) => {
  validateSender(event.sender, event.senderFrame);
  return SearchResponseSchema.parse(
    core.search(SearchRequestSchema.parse(p).query),
  );
});
...
ipcMain.handle("import:select-source", async (event, rawRequest: unknown) => {
  ...
  if (activeWorkspace.kind !== "local") {
    return ImportPreviewResultSchema.parse(
      importSessions.createPreview("", activeWorkspace),
    );
  }
  const owner = BrowserWindow.fromWebContents(event.sender);
  const options: OpenDialogOptions = {
    title:
      request.kind === "markdown-file"
        ...
        : "選擇要匯入的文章資料夾",
    properties:
      request.kind === "markdown-file" ? ["openFile"] : ["openDirectory"],
    ...(request.kind === "markdown-file"
