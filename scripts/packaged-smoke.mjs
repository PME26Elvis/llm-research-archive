...
if (!executable)
  throw new Error(`packaged executable not found: ${candidates.join(", ")}`);
...
try {
  ...
  await page.waitForFunction(
    () =>
      Number(
        document
          .querySelector('[data-testid="app-ready"]')
          ?.getAttribute("data-article-count"),
      ) > 0,
    undefined,
    { timeout: 30000 },
  );
  const count = Number(
    await page.getAttribute('[data-testid="app-ready"]', "data-article-count"),
  );
  const title =
    "科技業龍頭為何認為「現在算力遠遠不夠」：原因、證據與三大主題深度分析";
  await page.getByLabel("搜尋文章").fill("算力遠遠不夠");
  await page
    .getByRole("button", { name: new RegExp(title.slice(0, 12)) })
    .click();
  ...
  if (
    await page.evaluate(
      () => Reflect.has(window, "require") || Reflect.has(window, "process"),
    )
  ) {
  ...
  const version = await page
    .locator('[data-testid="about-version"]')
    .innerText();
  const expectedVersion = JSON.parse(
    fs.readFileSync("package.json", "utf8"),
  ).version;
  if (!commit || commit === "local")
    throw new Error(`packaged About commit is invalid: ${commit}`);
  if (version !== expectedVersion) {
    throw new Error(
      `packaged About version ${version} does not match ${expectedVersion}`,
    );
  }
  console.log(
