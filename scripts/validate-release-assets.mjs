...
if (!forge.includes("windowsSetupName()")) {
  throw new Error(
    "Forge Squirrel setup filename must come from windowsSetupName()",
  );
}
...
for (const name of artifacts) {
  if (!name.includes(version))
    throw new Error(`artifact name missing package version: ${name}`);
}
for (const name of [
  ...
  if (!artifacts.includes(name))
    throw new Error(`missing macOS artifact name ${name}`);
}
if (releaseAssetNames(version).length !== 10) {
  throw new Error(
    "release must contain seven binaries and three metadata assets",
  );
}
if (
  windowsSetupName(version) !==
...
for (const invalid of ["01.0.0", "1.0", "1.0.0-", "1.0.0-alpha..1"]) {
  if (validateSemver(invalid))
    throw new Error(`invalid SemVer accepted: ${invalid}`);
}
...
if (first.version !== version || first.source !== "package-version") {
  throw new Error(
    `unexpected first automatic version: ${JSON.stringify(first)}`,
  );
}
...
if (
  nextPatch.version !== semver.inc(version, "patch") ||
  nextPatch.source !== "auto-next-patch"
) {
  throw new Error(
...
if (!collisionRejected)
  throw new Error("explicit release version collision was not rejected");
