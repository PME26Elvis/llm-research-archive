...
if (artifacts.length !== 7) throw new Error(`expected seven binary artifacts, got ${artifacts.length}`);
...
const first = selectReleaseVersion({ packageVersion: version, existingTags: [] });
...
const nextPatch = selectReleaseVersion({ packageVersion: version, existingTags: [`v${version}`] });
if (nextPatch.version !== semver.inc(version, 'patch') || nextPatch.source !== 'auto-next-patch') {
  throw new Error(`automatic version did not advance after collision: ${JSON.stringify(nextPatch)}`);
}
const higher = selectReleaseVersion({ packageVersion: version, existingTags: ['v9.4.2'] });
if (higher.version !== '9.4.3') throw new Error(`unexpected highest-tag increment: ${higher.version}`);
