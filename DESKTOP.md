...
- 介面語言可選跟隨系統、繁體中文或 English；切換立即生效並同步原生選單與對話框，文章本文不會自動翻譯。
...
- Electron Forge Windows／Linux／macOS packaging、release manifest、packaged smoke、collision-free version selection、結構化 Markdown 變更摘要，以及可驗證的 draft／stable release pipeline。
...
- 語言偏好保留在 renderer；main 僅接收 schema-validated 的 resolved locale，以維持原生選單與對話框 authority。
...
4. `release_notes` 可留白，也可輸入簡短變更項目；逗號、分號、換行與貼上的 Markdown bullets 都會正規化成 `## What's changed` 清單。
5. 初次驗證使用 `channel=prerelease`、`publish=false`，先建立 draft；要直接發布則設 `publish=true`。
6. 檢查 Windows、Linux、macOS arm64／x64 assets、checksums、manifest、SBOM 與 GitHub Release 描述。
7. 要發布同一 draft 時，在 `requested_version` 填入 draft 的精確版本、保持相同 `target_ref`，再設 `publish=true`。
...
`release_notes` 最多 20 項、每項 200 字元、總輸入 2400 字元；重複項目會移除。Workflow 在昂貴的跨平台 build 前先驗證輸入，建立與刷新 draft 都使用同一份 Markdown notes file。
